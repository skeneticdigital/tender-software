...
export const BillingDashboard: React.FC = () => {
  ...
  const handleDownloadBill = (bill: Bill) => {
    ...
    const content = `=====================================================
            TENDERFLOW ERP - CLIENT INVOICE
=====================================================
Bill Number     : ${billNum}
Project Name    : ${bill.projectName}
Client Name     : ${bill.clientName}
Bill Date       : ${bill.billDate || '2026-05-05'}
Due Date        : ${bill.dueDate || '2026-06-05'}
Bill Status     : ${bill.status}
-----------------------------------------------------
Gross Bill Amount : ₹${(bill.grossAmount || 0).toLocaleString('en-IN')}
Total Deductions  : ₹${(bill.totalDeductions || 0).toLocaleString('en-IN')}
Net Payable       : ₹${(bill.netPayable || bill.netAmount || 0).toLocaleString('en-IN')}
Outstanding Bal   : ₹${(bill.outstandingAmount || 0).toLocaleString('en-IN')}
-----------------------------------------------------
Statutory Deductions Breakdown:
  - TDS (Income Tax 194C 2%) : ₹${((bill.grossAmount || 0) * 0.02).toLocaleString('en-IN')}
  - GST TDS (2%)             : ₹${((bill.grossAmount || 0) * 0.02).toLocaleString('en-IN')}
  - Labour Welfare Cess (1%) : ₹${((bill.grossAmount || 0) * 0.01).toLocaleString('en-IN')}
  - Retention Guarantee (5%) : ₹${((bill.grossAmount || 0) * 0.05).toLocaleString('en-IN')}
=====================================================
Generated by Elvina Infra Pvt Ltd - TenderFlow ERP System
`;
    ...
    link.href = url;
    link.download = `Invoice_${billNum.replace(/[/\\?%*:|"<>]/g, '_')}.txt`;
